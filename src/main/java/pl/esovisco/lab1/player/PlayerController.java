...
public class PlayerController {
    ...
    @GetMapping("{id}/clubs")
    public ResponseEntity<GetClubResponse> getPlayersClub(@PathVariable long id){
        Optional<Player> player = playerService.find(id);
        return player.map(value -> ResponseEntity.ok(GetClubResponse
                                .toClubResponse(value.getClub())))
                .orElseGet(() -> ResponseEntity.notFound().build());
    }
